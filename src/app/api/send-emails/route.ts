...
export async function POST(request: NextRequest) {
  try {
    ...
    // Type assertion for the event data with joined tables
    const event = eventData as any;

    // Get organizer email from auth
    const {
      data: { user: organizerUser },
    } = await supabase.auth.admin.getUserById(event.organizer_id);
      ...
      try {
        ...
        if (
          ...
          mailOptions.attachments = await Promise.all(
            event.event_files.map(async (file: any) => {
        ...
        console.error(
          `Error sending email to ${preview.partnerEmail}:`,
          emailError
        );
